const { BN, constants, expectEvent, expectRevert, singletons } = require("@bininvest/bininvest-test-helpers");
const { ZERO_ADDRESS } = constants;

const { expect } = require("chai");

const {
    shouldBehaveLikeBIXDirectSendBurn,
    shouldBehaveLikeBIXOperatorSendBurn,
    shouldBehaveLikeBIXUnauthorizedOperatorSendBurn,
    shouldBehaveLikeBIXInternalMint,
    shouldBehaveLikeBIXSendBurnMintInternalWithReceiveHook,
    shouldBehaveLikeBIXSendBurnWithSendHook,
} = require("./BIX.behavior");
...
const BIX = artifacts.require("BIXMock");
const BIXSenderRecipientMock = artifacts.require("BIXSenderRecipientMock");

contract("BIX", function (accounts) {
    const [registryFunder, holder, defaultOperatorA, defaultOperatorB, newOperator, anyone] = accounts;

    const initialSupply = new BN("10000");
    const name = "BIXTest";
    const symbol = "BIXT";
    const data = web3.utils.sha3("OZBIXTestData");
    const operatorData = web3.utils.sha3("OZBIXTestOperatorData");
    ...
    context("with default operators", function () {
        beforeEach(async function () {
            this.token = await BIX.new(holder, initialSupply, name, symbol, defaultOperators);
        });

        describe("as an TRC20 token", function () {
            shouldBehaveLikeTRC20("BIX", initialSupply, holder, anyone, defaultOperatorA);

            describe("_approve", function () {
                shouldBehaveLikeTRC20Approve("BIX", holder, anyone, initialSupply, function (owner, spender, amount) {
                ...
                describe("when the owner is the zero address", function () {
                    it("reverts", async function () {
                        await expectRevert(this.token.approveInternal(ZERO_ADDRESS, anyone, initialSupply),
                            "BIX: approve from the zero address",
                        );
                    });
                });
            });
...
a
            ...
            it("the BIXToken interface is registered in the registry", async function () {
                expect(await this.TRC1820.getInterfaceImplementer(this.token.address, web3.utils.soliditySha3("BIXToken")))
        ...
        context("with no BIXTokensSender and no BIXTokensRecipient implementers", function () {
            describe("send/burn", function () {
                shouldBehaveLikeBIXDirectSendBurn(holder, anyone, data);

                context("with self operator", function () {
                    shouldBehaveLikeBIXOperatorSendBurn(holder, anyone, holder, data, operatorData);
                });

                context("with first default operator", function () {
                    shouldBehaveLikeBIXOperatorSendBurn(holder, anyone, defaultOperatorA, data, operatorData);
                });

                context("with second default operator", function () {
                    shouldBehaveLikeBIXOperatorSendBurn(holder, anyone, defaultOperatorB, data, operatorData);
                });

                context("before authorizing a new operator", function () {
                    shouldBehaveLikeBIXUnauthorizedOperatorSendBurn(holder, anyone, newOperator, data, operatorData);
                });
                ...
                context("with new authorized operator", function () {
                    ...
                    shouldBehaveLikeBIXOperatorSendBurn(holder, anyone, newOperator, data, operatorData);
            ...
            describe("mint (internal)", function () {
                ...
                context("with default operator", function () {
                    ...
                    shouldBehaveLikeBIXInternalMint(to, operator, amount, data, operatorData);
                });
            ...
            describe("mint (internal extended)", function () {
                ...
                context("to non BIXTokensRecipient implementer", function () {
                    beforeEach(async function () {
                        this.tokensRecipientImplementer = await BIXSenderRecipientMock.new();
                    ...
                    context("with default operator", function () {
                        ...
                        it("with requireReceptionAck", async function () {
                            await expectRevert(
                                ...
                                "BIX: token recipient contract has no implementer for BIXTokensRecipient",
                            );
                        });
                    });
        ...
        describe("operator management", function () {
            ...
            it("reverts when self-authorizing", async function () {
                await expectRevert(
                    this.token.authorizeOperator(holder, { from: holder }), "BIX: authorizing self as operator",
                );
            });

            it("reverts when self-revoking", async function () {
                await expectRevert(
                    this.token.revokeOperator(holder, { from: holder }), "BIX: revoking self as operator",
                );
            });
        ...
        describe("send and receive hooks", function () {
            ...
            describe("tokensReceived", function () {
                ...
                context("with no BIXTokensRecipient implementer", function () {
                ...
                context("with BIXTokensRecipient implementer", function () {
                    context("with contract as implementer for an externally owned account", function () {
                        beforeEach(async function () {
                            ...
                            await this.TRC1820.setInterfaceImplementer(
                                this.recipient,
                                web3.utils.soliditySha3("BIXTokensRecipient"), this.tokensRecipientImplementer.address,
                                { from: this.recipient },
                            );
                        });

                        shouldBehaveLikeBIXSendBurnMintInternalWithReceiveHook(operator, amount, data, operatorData);
                    });

                    context("with contract as implementer for another contract", function () {
                        beforeEach(async function () {
                            this.recipientContract = await BIXSenderRecipientMock.new();
            ...
            describe("tokensToSend", function () {
                ...
                context("with a contract as implementer for an externally owned account", function () {
                    beforeEach(async function () {
                        this.tokensSenderImplementer = await BIXSenderRecipientMock.new();
                        this.sender = holder;
                        ...
                        await this.TRC1820.setInterfaceImplementer(
                            this.sender,
                            web3.utils.soliditySha3("BIXTokensSender"), this.tokensSenderImplementer.address,
                            { from: this.sender },
                        );
                    });

                    shouldBehaveLikeBIXSendBurnWithSendHook(operator, amount, data, operatorData);
                });

                context("with contract as implementer for another contract", function () {
                    beforeEach(async function () {
                        this.senderContract = await BIXSenderRecipientMock.new();
    ...
    context("with no default operators", function () {
        beforeEach(async function () {
            this.token = await BIX.new(holder, initialSupply, name, symbol, []);
        });
